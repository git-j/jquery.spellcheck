 ...
 */
 BJSpell = function(){
     ...
     * @return Boolean false if the word does not exist
     */
    BJSpell.prototype.replaceDOM = function(dom_node, callback){
        var self = this;
        var replacements = [];
        var cur_checks = 0, max_checks = 10; // prevent checking tables with many rows
        var replaceTextNodes = function(node) {
          var whitespace = /^\s*$/;
          if (!node ) return;
          if (cur_checks>max_checks) return;
          cur_checks = cur_checks + 1;
          if (node.nodeType == 3) {
            if (!whitespace.test(node.nodeValue)) {
              var new_nodeValue = self.replaceString(node.nodeValue,callback);
              replacements.push(function(){$(node).replaceWith(new_nodeValue);});
            }
          } else {
     ...
     * @return Boolean false if the word does not exist
     */
    BJSpell.prototype.replaceString = function(String, callback){
        var self = this;
        var max_checks = 100;
        var cur_checks = 0;
        return String.replace(self.alphabet, function(word){
            if ( cur_checks >= max_checks )
              return word;
            cur_checks+=1;
